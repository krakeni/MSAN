...
{
  int s, fd, rc;
  char buf[1500], *filename;
  size_t len;
  ssize_t c_read, c_send;
  struct stat s_stat;
  struct sockaddr_in server;

  if (!path || !address || port < 0 || port > 65535)
  {
    fprintf(stderr, "upload_file invalid arg!\n");
    return -1;
  }

  if ((rc = stat(path, &s_stat)) != 0)
  {
  ...
  {
    perror("Unable to connect on server");
    return -1;
  }
  ...
  buf[1] = 5; /* Msg type */
  buf[2] = 0; /* Status code */
  buf[3] = rush_digest_type_sha256; /* Digest type */

  memcpy(&buf[4], &s_stat.st_size, 4);
  filename = basename(path);
  len = strlen(filename);
  memcpy(&buf[8], &len, 4);
  memcpy(&buf[12], filename, len);

  if ((c_send = send(s, &buf, 12 + len, 0)) == -1)
  {
    perror("Unable to send on header buff");
    return -1;
  }

  memset(buf, 0, 1500);

  get_hash_sha256(path, &buf[0]);

  if ((c_send = send(s, &buf, 64, 0)) == -1)
  {
    perror("Unable to send on digest buff");
    return -1;
  }

  if (c_send != 64)
  {
    fprintf(stderr, "ERROR IN SENDING HASH\n");
  }

  memset(buf, 0, 1500);

  while ((c_read = read(fd, &buf, 1500)))
  {
    if (c_read == -1)
    {
    ...
    if ((c_send = send(s, &buf, c_read, 0)) == -1)
    {
      perror("Unable to send on buff");
      return -1;
    }

    if (c_send != c_read)
      printf("Error: read size: %zu send size: %zu\n", c_read, c_send);
  }
